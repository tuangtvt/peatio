...
describe Peatio::Core::Ranger do
  before(:all) do
    Peatio::Core::MQ::Client.connection = BunnyMock.new.start
  end

  let(:logger) { Peatio::Core::Logger }
  ...
  let(:auth) {
    Peatio::Core::Auth::JWTAuthenticator.new(jwt_public_key, jwt_private_key)
  }
  let(:router) { Peatio::Core::Ranger::Router.new }
  let(:ex_name) { "peatio.events.ranger" }

  let(:logger) {
    Peatio::Core::Logger.logger
  }
  ...
  context "invalid token" do
    let!(:client) { Peatio::Core::MQ::Client.new }
    ...
    it "denies access" do
      em {
        ...
        ws_server do |socket|
          connection = Peatio::Core::Ranger::Connection.new(router, socket, logger)
  ...
  context "valid token" do
    ...
    it "sends messages that belong to the user and filtered by stream" do
      em {
        ...
        Peatio::Core::Ranger.run(jwt_public_key, ex_name, ranger_port: 88888)
